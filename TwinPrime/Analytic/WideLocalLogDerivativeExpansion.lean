...
/-!
# A local zero expansion on the closed disk of radius 17/16

The actual divisor is still taken on radius `5/4`, inside the analytic
outer disk of radius `3/2`. Enlarging the evaluation disk only doubles
the normalized logarithmic remainder constant from `144` to `288`.
-/
...
theorem wide_norm_logDeriv_sub_zero_sum_le {f : ℂ → ℂ} {c : ℂ} {M : ℝ}
    (hf : AnalyticOnNhd ℂ f (closedBall c (3 / 2))) (hfc : f c ≠ 0)
    (hM : 1 ≤ M) (hbound : ∀ w ∈ sphere c (3 / 2), ‖f w‖ ≤ M)
    (z : ℂ) (hz : z ∈ closedBall c (17 / 16)) (hfz : f z ≠ 0) :
    ‖logDeriv f z - (∑ᶠ u, (divisor f (closedBall c (5 / 4)) u : ℂ) / (z - u))‖ ≤
      (288 * (1 + Real.log 5 / Real.log (6 / 5))) * (1 + Real.log (M / ‖f c‖)) := by
  ...
  have hgz := wide_norm_logDerivative_le_of_nonvanishing_on_ball hgball hgnball hgbound z hz
  ...
  exact (mul_le_mul_of_nonneg_left hbudget (by norm_num)).trans
    (wide_zero_removal_log_budget_le_of_jensen N hN)
